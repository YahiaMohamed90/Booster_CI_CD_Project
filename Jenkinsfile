pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                sh 'docker build -f dockerfile . -t yehiam/jenkins_python_dev:v1.0'
            }
        }
        stage('Push') {
            steps {
                ...
                sh 'docker push  yehiam/jenkins_python_dev:v1.0'
            }
        ...
        stage('Deploy') {
            steps {
                sh 'docker run -d -p 6060:5050 yehiam/jenkins_python_dev:v1.0'
            }
